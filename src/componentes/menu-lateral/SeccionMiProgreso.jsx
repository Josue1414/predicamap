...
import { Target, Clock, BookOpen, Award, Sprout, Flower2, Gift, PartyPopper, ChevronDown, ChevronUp, Star, Calendar as CalIcon } from 'lucide-react';
...
export default function SeccionMiProgreso({ perfilUsuario, acordeonActivo, alternarAcordeon }) {
  const {
    ...
    modificarHorasHoy, setEstudiosHoy, actualizarMetas, fechaHoyStr
  } = useGestorProgreso();
  ...
  // Guardamos el progreso real (puede superar el 100%) para los iconos
  const progresoReal = (horasMesActual / metaMensualUI) * 100;
  // Limitamos a 100% solo para la barra visual
  ...
  // Función para renderizar los iconos de forma dinámica y tierna
      ...
      {acordeonActivo === 'progreso' && (
        <div className="p-3 bg-slate-50/50 dark:bg-slate-950">
          <div className="space-y-5 animate-slide-up">
            ...
            {/* TARJETA CENTRAL: PROGRESO Y CONTROLES */}
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center">
                  <div className="flex items-center gap-2 mb-1.5">
                    <button onClick={() => modificarHorasHoy(-1)} className="w-8 h-8 rounded-full bg-rose-100 dark:bg-rose-900/30 text-rose-500 font-black flex items-center justify-center hover:scale-110 active:scale-95 transition-all">-1</button>
                    <button onClick={() => modificarHorasHoy(1)} className="w-12 h-12 rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 font-black text-lg flex items-center justify-center shadow-md hover:scale-105 active:scale-95 transition-all">+1</button>
                  </div>
                  <span className="text-[10px] font-bold text-slate-500">Horas de Hoy</span>
                </div>
                
                <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 flex flex-col items-center justify-center">
                  <div className="flex items-center gap-2 mb-1.5">
                    <button onClick={() => setEstudiosHoy(estudiosMesActual - 1)} className="p-1 text-slate-400 hover:text-slate-600">-</button>
                    <div className="text-xl font-black text-slate-700 dark:text-slate-200 flex items-center gap-1.5"><BookOpen size={16} className="text-emerald-500" /> {estudiosMesActual}</div>
                    <button onClick={() => setEstudiosHoy(estudiosMesActual + 1)} className="p-1 text-slate-400 hover:text-slate-600">+</button>
                  </div>
                  <span className="text-[10px] font-bold text-slate-500">Estudios</span>
                </div>
              </div>
            </div>

            {/* WIDGET DE CALENDARIO NATIVO */}
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 border border-slate-200 dark:border-slate-800">
              <h4 className="font-bold text-xs text-slate-700 dark:text-slate-300 flex items-center gap-2 mb-3"><CalIcon size={14} className="text-indigo-500" /> Mi Actividad del Mes</h4>
            ...
            <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-4 border border-slate-200 dark:border-slate-800">
              ...
              <div className="space-y-4">
                ...
                <div className="pt-3 border-t border-slate-200 dark:border-slate-800">
                  ...
                  {/* TEXTO DE SUGERENCIA EXACTA Y CONDICIONAL */}
                  {tieneMetaAnual && horasRestantesAnuales > 0 && (
                    <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 p-3 rounded-xl">
                      <p className="text-[10px] text-slate-600 dark:text-slate-400 leading-snug">
                        {horasTotalesAño > 0 
                          ? `Tomando en cuenta las ${horasTotalesAño} horas que ya llevas registradas, y que faltan ${diasHastaAgosto} días para el 31 de agosto, tendrías que hacer un promedio de `
                          : `Faltan exactamente ${diasHastaAgosto} días para el 31 de agosto. Para alcanzar tu meta tendrías que hacer un promedio de `}
                        <span className="font-black text-indigo-600 dark:text-indigo-400">{formatearTiempoTexto(metaDiariaSugerida)} al día</span>. Esto equivale a <span className="font-black text-emerald-600 dark:text-emerald-500">{metaMensualSugerida} horas al mes</span>. ¡Si mantienes este ritmo mensual sugerido, lograrás un total aproximado de <span className="font-black text-slate-800 dark:text-slate-200">{totalProyectado} horas</span>!
                      </p>
                    </div>
                  )}
                  {tieneMetaAnual && horasRestantesAnuales === 0 && (
                    <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 p-3 rounded-xl text-center">
                      <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400">
                        ¡Felicidades! Ya has completado o superado tu meta de {metaAnual} horas.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <details className="pt-2 border-t border-slate-200 dark:border-slate-800 group">
                  <summary className="text-[10px] font-bold text-blue-600 dark:text-purple-400 hover:text-blue-700 dark:hover:text-purple-300 transition-colors cursor-pointer list-none flex items-center gap-1">
                    <span className="transform group-open:rotate-90 transition-transform">▸</span> ¿Empezaste a usar la app a mitad de año?
                  </summary>
                  <div className="mt-3">
                    <label className="block text-[10px] font-bold text-slate-500 mb-1">Horas acumuladas antes de usar PredicaMap</label>
