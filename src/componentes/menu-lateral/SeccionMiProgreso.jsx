...
import { Target, Clock, BookOpen, Award, Sprout, Flower2, Gift, PartyPopper, ChevronDown, ChevronUp, Star, Calendar as CalIcon, AlertCircle, Download, Upload, ChevronLeft, ChevronRight, Info, FileText } from 'lucide-react';
  ...
  return (
    <div className="bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden mb-2">
      ...
      {acordeonActivo === 'progreso' && (
        <>
          <div className="bg-amber-50 dark:bg-amber-900/20 px-3 py-2 border-t border-b border-amber-100 dark:border-amber-900/50 flex items-start gap-2">
            <Info size={14} className="text-amber-600 dark:text-amber-500 mt-0.5 shrink-0" />
            <p className="text-[10px] font-bold text-amber-800 dark:text-amber-400 uppercase">Tu progreso solo vive en tu dispositivo, no se muestra a los demás.</p>
          </div>
          <div className="p-3 bg-slate-50/50 dark:bg-slate-950">
            <div className="space-y-5 animate-slide-up">
              ...
              {mostrarAvisoInforme && (
                <div className={`p-3 rounded-2xl border flex items-center gap-3 shadow-sm ${esUltimoDia ? 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/30 dark:border-indigo-800' : 'bg-emerald-50 border-emerald-200 dark:bg-emerald-900/30 dark:border-emerald-800'}`}>
                  ...
                  <div className="flex flex-col">
                    <span className={`text-[11px] font-black uppercase tracking-wide ${esUltimoDia ? 'text-indigo-700 dark:text-indigo-400' : 'text-emerald-700 dark:text-emerald-400'}`}>
                      {esUltimoDia ? '¡Excelente mes!' : '¡Aviso de informe!'}
                    </span>
                    <span className={`text-[10px] font-medium leading-tight mt-0.5 ${esUltimoDia ? 'text-indigo-600 dark:text-indigo-300' : 'text-emerald-600 dark:text-emerald-300'}`}>
                      {esUltimoDia 
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-3 border-t border-slate-200 dark:border-slate-800">
                    ...
                    {tieneMetaAnual && horasRestantesAnuales > 0 && (
                      <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 p-3 rounded-xl mt-3">
                    ...
                    {tieneMetaAnual && horasRestantesAnuales === 0 && (
                      <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 p-3 rounded-xl text-center mt-3">
